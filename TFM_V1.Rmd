...
**** Todos los "lm" superan de R2 un 0.9, excepto América que tiene un R2 muy bajo "0.01", porque sus valores tienen poca variabilidad. Se observa que la estimación que hace del indicador no es mala.
...
#### Se decide hace las estimaciones por regresión lineal
...
```{r}
dataset<-Reduce(function(...) merge (..., all.x=TRUE), list(m7,ANAEMIA_RW,ANAEMIA_PW,Life_expectancy_birth))
glimpse(dataset)
unique(dataset$Who_Region)
unique(dataset$year)
```
